...
const jobSeeker = computed(()=>user.value.Job_profile || {})
...
const submit = async()=>{
  ...
  try{
    ...
    if(jobSeeker.value?.id){
      await dataStore.updateItem('jobseekers',jobSeeker.value.id,form.value)
    }else{
