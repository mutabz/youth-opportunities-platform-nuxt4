...
<main class="user-dashboard container-fluid">
  ...
  <div class="row mt-2">
    ...
    <section class="col-lg-9">
      ...
      <!-- OPPORTUNITIES LIST -->
      <div v-if="filteredItems.length" class="grid-2">

        <div
          class="card shadow-lg border-0 rounded-4"
          v-for="opp in filteredItems"
          :key="opp.id"
        >
          ...
          <div class="card-footer">
            ...
            <button @click="SaveOpp(opp.id)" class="btn btn-outline-success rounded-4" style="font-size: 0.8rem; padding: 0.4rem 1rem;"><span v-if="loading && opp.id === SavingId">Saving....</span><span v-else>Save {{opp.category}}</span></button>
...
// Fetch data on component mount
async function SaveOpp(opp_id) { 
  loading.value = true
  try {
    SavingId.value = opp_id
    const data = {
      user_id: user.id,
      opportunity_id: opp_id
    }

    await dataStore.createItem('saved_opportunities', data) 
  loading.value = false
  } catch (e) {
    alert(e)
  }
