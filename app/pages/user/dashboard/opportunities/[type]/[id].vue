  ...
  <main class="user-dashboard p-3">
    ...
    <div class="row g-4">
      ...
      <div class="col-12" v-if="details.application?.application_link">
     	  <div class="d-flex justify-content-center" v-if="device === 'desktop'">
            <NuxtLink v-if="details.application.application_link" :href="details.application.application_link" target="_blank" class="btn btn-primary rounded-4 w-50  btn-lg mt-2">
                Apply for this {{ type }}
            </NuxtLink>

            <NuxtLink :to="'/opportunities/apply/' + opportunity.id" class="btn btn-success rounded-4 w-50 btn-lg mt-2" >
              Need Help For Application
            </NuxtLink>
        </div>
        <div class="" v-if="device !== 'desktop'">
            <button v-if="!user" @click="showLoginEnquiry = true" class="btn btn-primary rounded-4  w-100 btn-lg  ">
              Apply for this {{ type }}
            </button>

            <div class="" v-else>
              <a v-if="details.application.application_link" :href="details.application.application_link" target="_blank" class="btn btn-primary rounded-4  w-100 btn-lg mt-2">
                Apply for this {{ type }}
              </a>
            </div>

            <NuxtLink :to="'/opportunities/apply/' + opportunity.id" class="btn btn-success rounded-4 w-100 btn-lg  mt-2" >
              Need Help For Application
            </NuxtLink>
        </div>
      </div>
...
const filteredDetailSections = computed(() =>
  type.value
    ? detailSections.filter(s => s.type?.includes(type.value))
    : []
)

const filteredDynamicSections = computed(() =>
  type.value
    ? otherDynamicSections.filter(s => s.type?.includes(type.value))
    : []
)
...
// Watch opportunity to update SEO dynamically once data is loaded
watch(opportunity, (newOpp) => {
  ...
  useSeoMeta({
    ...
    meta: [
      { name: 'keywords', content: newOpp.details.slugs?.join(', ') || '' }
    ],
    script: [
      {
...
useHead(() => {
  ...
  return {
    ...
    meta: [
      {
        name: "description",
        content: opportunity.value.description?.[0] || generateMetaDescription(opportunity.value)
      },
      {
        name: "keywords",
        content: opportunity.value.tags?.join(', ') || ''
      }
    ],
    script: [
      {
