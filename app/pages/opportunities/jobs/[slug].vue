...
const post = computed(() => {
  return jobs.find(s => s.slug === slug) || {
    title: 'Scholarship Opportunities',
    description: 'Find jobs, jobs, jobs, and fellowships worldwide.',
    slug: 'default'
  }
})
...
useHead({
  ...
  meta: [
    ...
    { property: 'og:url', content: `${BASE_URL}/opportunities/jobs/${post.value.slug}` },
    { name: 'twitter:card', content: 'summary_large_image' },
    { name: 'twitter:title', content: post.value.title },
    { name: 'twitter:description', content: post.value.description }
  ],
  link: [
    { rel: 'canonical', href: `${BASE_URL}/opportunities/jobs/${post.value.slug}` }
  ],
  script: [
    {
      ...
      children: JSON.stringify({
        ...
        "url": `${BASE_URL}/opportunities/jobs/${post.value.slug}`,
        "provider": {
          "@type": "Organization",
          "name": "YOP Rwanda",
          "sameAs": BASE_URL
